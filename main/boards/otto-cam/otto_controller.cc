...
public:
    ...
    void RegisterMcpTools() {
        ...
        // 音频播放工具
        mcp_server.AddTool(
            "self.otto.play_sprunki_sound","播放节奏盒子的音乐。",PropertyList(),
            [this](const PropertyList& properties) -> ReturnValue {
                
                // 播放音乐
                Application::GetInstance().PlaySound(Lang::Sounds::P3_SPRUNKI);
                return true;
            });
             

        mcp_server.AddTool("self.camera.capture_and_analyze", "拍照并识别图像中的主要物体", PropertyList({Property("question", kPropertyTypeString, "识别图像中的主体是什么?")}), [this](const PropertyList &properties) -> ReturnValue
                           {
                            auto& board = Board::GetInstance();
                            Camera* camera_ = board.GetCamera();
                            // 先拍照
                            if (!camera_->Capture()) {
