...
private:
...
#ifdef CONFIG_OTTO_WEB_CONTROL_ENABLED
                                 ...
                                 int32_t event_id, void* event_data)
    {
        if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_CONNECTED) {
            xTaskCreate(
                [](void* arg) {
                    OttoRobot* instance = static_cast<OttoRobot*>(arg);
                    
                    vTaskDelay(5000 / portTICK_PERIOD_MS);

                    if (!instance->web_server_initialized_) {
                        ESP_LOGI(TAG, "WiFi连接成功，初始化Web控制服务器");
                        esp_err_t err = otto_web_server_init();
                        if (err != ESP_OK) {
                            ESP_LOGE(TAG, "Web控制服务器初始化失败: %d", err);
                        } else {
                            ESP_LOGI(TAG, "Web控制服务器初始化成功");
                ...
                "web_server_init",
                1024 * 8, arg, 5, nullptr);
        }
    }
#endif //CONFIG_OTTO_WEB_CONTROL_ENABLED
