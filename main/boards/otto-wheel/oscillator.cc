...
void Oscillator::SetSpeed(int speed) {
   // Clamp speed to -100 to 100 range
    rotation_speed_ = std::max(-100, std::min(100, speed));
        
    // Convert speed to servo position
    // For continuous rotation servos:
    // 90 = stop, <90 = counterclockwise, >90 = clockwise
    int servo_pos = 90 + (rotation_speed_ * 90 / 100);
    Write(servo_pos);
}

void Oscillator::StopRotation() {
    rotation_speed_ = 0;
    Write(90);  // Stop position for continuous rotation servo
}
